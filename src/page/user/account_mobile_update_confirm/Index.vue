<template>
  <div>
    <van-nav-bar v-bind:class="{ active: isActive }" title="输入验证码" right-text="确认" @click-left="$close()" @click-right="onClickRight" left-arrow />
    <div class="title">短信验证码已发送至 {{ mobile }}，请填写验证码</div>

    <van-cell-group>
      <van-field v-model="code" center clearable label="短信验证码" placeholder="请输入短信验证码" type="number">
  ...
  methods: {
    ...
    onClickRight() {
      if (this.isActive) {
        ...
        this.$http.put('account/bind_mobile', data).then(() => {
